...
const emptyAssistant = (): AssistantForm => ({ firstName: "", lastName: "", email: "", department: "" });
...
export default function AdminPage(): React.ReactElement {
	...
	const submitDoctor = async (e: React.SyntheticEvent<HTMLFormElement>): Promise<void> => {
		...
		setError("");
		try {
			if (modal === "create-doctor") await api.post("/admin/doctors", doctorForm);
			else await api.put(`/admin/doctors/${selected!.doctor?.id}`, doctorForm);
			closeModal();
			load();
		} catch {
	...
	const submitAssistant = async (e: React.SyntheticEvent<HTMLFormElement>): Promise<void> => {
		...
		setError("");
		try {
			if (modal === "create-assistant") await api.post("/admin/assistants", assistantForm);
			else await api.put(`/admin/assistants/${selected!.assistant?.id}`, assistantForm);
			closeModal();
			load();
		} catch {
			setError("Could not save. Please try again.");
